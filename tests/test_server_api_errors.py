...
class _HttpExplodingManager:
    async def add_account(self, apple_id: str, password: str, domain: str = "cn"):
        raise web.HTTPInternalServerError(text="boom-http")

    async def verify_2fa(self, apple_id: str, code: str):
        raise web.HTTPBadRequest(text="boom-bad-request")
...
class ApiHttpExceptionHandlingTests(unittest.IsolatedAsyncioTestCase):
    async def asyncSetUp(self):
        app = web.Application()
        app["manager"] = _HttpExplodingManager()
        app.router.add_post("/api/accounts/add", handle_add_account)
        app.router.add_post("/api/accounts/{account}/verify-2fa", handle_verify_2fa)
        self.client = TestClient(TestServer(app))
        await self.client.start_server()
    ...
    async def test_add_account_returns_json_when_http_exception_is_raised(self):
        resp = await self.client.post(
            "/api/accounts/add",
            json={"apple_id": "user@example.com", "password": "pw", "domain": "com"},
        )
        ...
        self.assertEqual(resp.content_type, "application/json")
        payload = await resp.json()
        self.assertEqual(payload["result"], "Server error: boom-http")

    async def test_verify_2fa_returns_json_when_http_exception_is_raised(self):
        resp = await self.client.post(
            "/api/accounts/user@example.com/verify-2fa",
            json={"code": "123456"},
        )
        ...
        self.assertEqual(resp.content_type, "application/json")
        payload = await resp.json()
        self.assertEqual(payload["result"], "Server error: boom-bad-request")
